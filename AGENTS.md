...
- A 0-flow mitm capture through the system HTTP proxy is not proof data stayed on-device. Flutter apps often ignore that proxy. This AVD has no tcpdump. Report file growth vs wall-clock soak, and mixed emulator-wide hosts vs app-only. Proof that Firebase was silent needs a reachable-host control, a packet capture the app cannot skip, a finished-profile time window, and two records that agree. See METHODOLOGY.md "What proves Firebase is silent".
