...
- Invoke superpowers and trainer throughout this harness work. On user-facing docs (CHANGELOG, README, FINAL-REPORT, ROADMAP, METHODOLOGY), also run peitho and deai before commit.
...
- Keep the emulator window visible so the operator can see it while a live session is running. Use emulator-5554 only; ignore emulator-5556 for work, but still shut leftover qemu down. Do not restart a live windowed session into headless. When emulator work for the session or slice is done, always shut it down (`adb -s emulator-5554 emu kill`, and `adb -s emulator-5556 emu kill` if a leftover is still listed) so qemu does not keep burning CPU. Do not leave qemu running overnight or between slices.
...
- Google account sign-in fails while the mitm proxy is on (cert pinning). Turn the proxy off for Google auth; restore `10.0.2.2:8080` before capture. After capture set proxy `:0`. SIGINT mitmdump; confirm `lsof` shows port 8080 free before a new dump (the child may outlive the wrapper). Do not start mitmdump inside a short safe-cmd wrapper (it kills the process group and can leave a 0-byte `.mitm` that must stay).
...
- Baby+ About You CONTINUE sends a first-party PUT to `appserver.health-and-parenting.com`; a scan that skips HTTP request bodies can miss the name and report no transmission. Wipe `com.hp.babyapp` data to show About You again. After About Baby DONE, a force-upgrade / Play Store gate can block home; do not tap GO TO PLAYSTORE unless a real Play Store is present.
- A 0-flow mitm capture through the system HTTP proxy is not proof data stayed on-device. Flutter apps often ignore that proxy. This AVD has no `eth0`. After `adb root`, `/system/bin/tcpdump` on `wlan0` works. Report file growth vs wall-clock soak, and mixed emulator-wide hosts vs app-only. Proof that Firebase was silent needs a reachable-host control, a packet capture the app cannot skip, a finished-profile time window, and two records that agree. See METHODOLOGY.md "What proves Firebase is silent".
- Use one synthetic baby: Privatia Rigatoni; alias-first (same value under another field name is not a new field). Baby Buddy has no APK under `apks/`; a missing APK is not a privacy verdict.
- Most captures are a short first-launch window. That is not a 20-minute idle, and it is not Backup/Sign-in/device-pair. Nubo 2026-08-18 finished home activities on device. 2026-08-23: Google backup login with proxy off; Backup Now hung on GMS spinner. 2026-08-24: Backup Now finished after GMS Continue; last backup `08/24/2026 18:49:05`; BLE pair still not tapped. See ROADMAP.md "App surface and sync-condition coverage".
