...
| Baby+ | com.hp.babyapp | done | Logged in. About You (2026-08-21 clear + Google). About Baby name + Girl + DONE. Then MainActivity Important Update (GO TO PLAYSTORE). Gender TalkBack dump still has no Boy/Girl nodes; Girl is in a PopupWindow | Recipe taps spinner then Girl then `done_button` (`inject-config/com.hp.babyapp.json`) | `transmission_observed` on 2026-08-21 About You PUT (`firstName` to maker server). Earlier 2026-08-19 soak was a false negative before HAR `postData` fix. Force-upgrade still blocks home. |
...
2. Baby+ About You picture and capture are done (2026-08-21). Scan after HAR `postData` fix: `transmission_observed` on parent-name PUT. Force-upgrade still blocks home. Re-scan older `.mitm` files if you need request-body verdicts from before that fix.
