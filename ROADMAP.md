...
| Baby+ | com.hp.babyapp | done | Logged in. About Baby name + Girl + DONE (2026-08-19). Then MainActivity Important Update (GO TO PLAYSTORE). Gender TalkBack dump still has no Boy/Girl nodes; Girl is in a PopupWindow | Recipe taps spinner then Girl (`inject-config/com.hp.babyapp.json`) | no_transmission_detected on 25-flow post-DONE upgrade soak; first 0-byte file kept |
