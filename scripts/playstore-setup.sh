...
print(g.select_apk_path(os.environ['LISTING'], os.environ['APKNAME']) or '')")
    ...
    # Android 10 kills system_server when a priv-app requests permissions that
    # are not in its privapp-permissions whitelist; the google_apis image ships
    # none for com.android.vending, so push one for the Play build installed here.
    local whitelist
    whitelist="$work/privapp-permissions-phonesky.xml"
    cat > "$whitelist" <<'XML'
...
XML
    adb_sel push "$whitelist" "/system/etc/permissions/privapp-permissions-phonesky.xml" >/dev/null
    adb_sel shell "chmod 644 /system/etc/permissions/privapp-permissions-phonesky.xml"
    log "pushed privapp-permissions whitelist for com.android.vending"
    rm -rf "$work"
