...
- **Live check (2026-08-23 backup):** Proxy off, then Sign in with Google, then the device account picker (`thebabyaudit@gmail.com`). Settings showed Logout. Formula-per-click stayed 15 (90 chip `selected=false` after tap and after Appium click; `viewMap1` covers the chip row). Backup Now then Yes left a spinner. `ConsentActivity` had no dump nodes. HTTP-proxy file `results/nubo-test-20260823/artifacts/captures/Nubo-backup-google.mitm` is 0 bytes and was kept. `wlan0` pcap `nubo-backup-wlan0.pcap` is 41518 bytes and shows TLS to Google IPs during that hang. Last backup text still said you have not backed up yet. This is not a finished cloud backup and not a Firebase-silence result.
