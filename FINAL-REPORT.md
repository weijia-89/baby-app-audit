...
- When an app only offers a short list (chips, pickers, units), we pick from those options. We prefer the profile sentinel when it sticks. If it does not, we use a value that does stick. The Final Report names the exact number or unit that was saved and whether that string left the device. A fixed target such as formula-per-click 90 is not required for a finished inject.
- After capture, `scripts/scan-synthetic-baby-data.sh` greps the raw local capture for the profile's marker strings. It reports which fictional values appear in a request body, a response body, or a request URL, and the recipient host for each. When the entered amount or unit differs from the profile sentinel, we also search for that exact string and report the result here.
...
Status: the profile, the scan tool, and a unit test are in place. Live inject+scan continues for apps that still lack a finished-use capture. How much of each app we actually used, and whether we waited for a delayed sync, is in ROADMAP.md (App surface and sync-condition coverage). Screenshots of every prior test are also on that list. Baby+ (2026-08-16): Google sign-in reached **About You** / **About Baby**. Capture `results/baby-plus-test-20260816/artifacts/captures/Baby+-about-you.mitm` (1 flow). Re-scan after HAR `postData` fix (2026-08-23): `transmission_observed` - high-confidence name marker in a **request** PUT to `appserver.health-and-parenting.com`. Baby+ (2026-08-19): name Privatia Rigatoni, Girl, **DONE**. MainActivity then showed Important Update (GO TO PLAYSTORE). First proxy file stayed 0 bytes (mitmdump died with the start script). Second file `results/baby-plus-test-20260819/artifacts/captures/BabyPlus-post-done-upgrade-soak.mitm`: 25 flows, 716,753 bytes. Grew at 19:00 and 19:12; no growth after 19:12. About You was not on screen that day. Re-scan after the HAR fix still says `no_transmission_detected` (name only in a maker **response**, not a request or URL). Baby+ (2026-08-21): cleared app data, Google login with proxy off, About You PNG (`results/baby-plus-test-20260821/artifacts/uiux/article-about-you.png`), parent name Privatia Rigatoni, CONTINUE, About Baby Girl + DONE, force-upgrade again. Capture `BabyPlus-about-you-full.mitm`: 14 flows, 589,030 bytes. Grew during inject; flat for 21 minutes on the upgrade screen. Scan: `transmission_observed`. Name marker in a **request** PUT to `appserver.health-and-parenting.com`, and again in a maker-server **response**. Hosts are whole-emulator proxy (maker, ads, S3), not Baby+-only. Kept two 0-byte mitm starts from the same day. Sanitized log: [network-log-baby-plus.json](results/network-log-baby-plus.json). This still is not proof Firebase was silent. MimiLog (2026-08-17): local save; 0 system-proxy flows; scan `no_transmission_detected`.[^mimilog-play] Amila (2026-08-17): name saved on the home screen. Capture had 9 flows. Scan: `no_transmission_detected` (0 marker hits). Baby Daybook (2026-08-17): crashed at Pairip native load on this emulator. No inject. Pebbi (2026-08-17): 4.0.1 reached **Add New Baby** then Pairip on cold start. Pulled 3.2.1 (forced update), 3.4.0 and 3.5.0 (Pairip). No saved Pebbi profile. Nurture Lock: Pairip **CLOSE** only. Nubo (2026-08-18 evening): inject of finished milk/sleep/pump/bottle/pee/poop/note on the saved profile. The system HTTP proxy stayed up about 21 minutes after save. The new file `results/nubo-test-20260818-soak/artifacts/captures/Nubo-finished-use-soak.mitm` has 15 flows and did not grow after 18:58. Scan `no_transmission_detected`. Marker hits were low-confidence numbers in **responses** (including a host control GET to Firebase that returned 404), not the baby name or note in a request. Those 15 flows are the whole emulator proxy, including Play, GMS, YouTube, and the host control, not Nubo-only. The 0-byte `results/nubo-test-20260817/artifacts/captures/Nubo.mitm` was kept. This still is not proof Firebase was silent.
...
- **Live check (synthetic markers):** About You PUT carries the parent name on 2026-08-16 and 2026-08-21 (`transmission_observed` after HAR `postData` fix). See the Synthetic baby-data section above. Force-upgrade still blocks home after About Baby DONE.
- **Accessibility (2026-08-16 and 2026-08-19, API 29 emulator, logged-in onboarding):** On **About Baby**, gender is required (`Please select baby’s gender`). The control is `com.hp.babyapp:id/baby_1_gender_options` (`CustomMaterialSpinner`). The dump text is only `Baby's Gender`. `content-desc` is empty. Boy and Girl are not nodes in the uiautomator dump. They appear in a `PopupWindow` (frame `[63,971][1025,1219]` on this AVD). A tap on the right edge of the spinner opens Boy/Girl. Girl is the lower row. 2026-08-19: we selected Girl and tapped **DONE**. The app opened MainActivity with Important Update / GO TO PLAYSTORE. Local dump from 2026-08-16: `results/baby-plus-test-20260816/artifacts/uiux/about-baby-gender.xml`. Pictures from 2026-08-19 stay under `results/baby-plus-test-20260819/artifacts/uiux/` (gitignored).
...
- **Live check (2026-08-18 evening soak):** Same finished inject on Privatia Rigatoni (milk L/R start+stop, sleep start+stop, pump start+stop, bottle, pee, poop, note save). Then Settings and Backup & Restore. Backup opened **Sign in with Google** with text "Please login to backup data". We skipped sign-in while the proxy was on. Formula-per-click stayed **15**. The proxy stayed up about 21 minutes. The file `Nubo-finished-use-soak.mitm` has 15 flows and did not grow after 18:58. Scan `no_transmission_detected`. No baby-name or note string in a request or URL. The 15 flows are all emulator HTTP-proxy traffic, including Play, GMS, YouTube, a 404 GET to `firebaseinstallations.googleapis.com` from a host control, and a failed CONNECT to `app-measurement.com`. Idle minutes after 18:58 added no new proxy bytes. A 15-flow system-proxy file is not the Firebase-silence bar. The 0-byte 2026-08-17 `Nubo.mitm` was not overwritten.
- **Live check (2026-08-23 backup):** Proxy off, then Sign in with Google, then the device account picker (`thebabyaudit@gmail.com`). Settings showed Logout. Formula-per-click entered as **15** (preferred sentinel 90 chip stayed `selected=false` after tap and after Appium click; `viewMap1` covers the chip row). That entered value is what to name in findings and to search for in the capture; missing 90 is not an unfinished inject. Backup Now then Yes left a spinner. `ConsentActivity` had no dump nodes. HTTP-proxy file `results/nubo-test-20260823/artifacts/captures/Nubo-backup-google.mitm` is 0 bytes and was kept. `wlan0` pcap `nubo-backup-wlan0.pcap` is 41518 bytes and shows TLS to Google IPs during that hang. Last backup text still said you have not backed up yet. This is not a finished cloud backup and not a Firebase-silence result.
- **Live check (2026-08-24 Backup Now):** Same signed-in session (Logout still on Settings). Proxy stayed `:0`. Backup Now, Yes, then GMS Drive consent. After scroll, Continue had real dump bounds. After Continue, last backup time was `08/24/2026 18:49:05`. Formula stayed **15**. Pictures under `results/nubo-test-20260824/artifacts/uiux/` (gitignored). `wlan0` pcap `results/nubo-test-20260824/artifacts/captures/nubo-backup-wlan0.pcap` is 1339264 bytes with TLS to Google IPv4s (15 unique `:443` endpoints). No HTTP-proxy `.mitm` this pass. The 2026-08-03 FAIL mark does not change. This is not the Firebase-silence bar.
