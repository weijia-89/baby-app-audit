...
Status: the profile, the scan tool, and a unit test are in place. Live inject+scan is in progress. How much of each app we actually used, and whether we waited for a delayed sync, is in ROADMAP.md (App surface and sync-condition coverage). Screenshots of every prior test are also on that list. Baby+ (2026-08-16): Google sign-in reached **About You** / **About Baby**. Parent-name **CONTINUE** sent a `PUT` to `appserver.health-and-parenting.com` with no plaintext marker match. Baby name was entered; **DONE** did not complete because gender is not exposed to automation (see Baby+ accessibility note). MimiLog (2026-08-17): local save; 0 system-proxy flows; scan `no_transmission_detected`.[^mimilog-play] Amila (2026-08-17): name saved on the home screen. Capture had 9 flows. Scan: `no_transmission_detected` (0 marker hits). Baby Daybook (2026-08-17): crashed at Pairip native load on this emulator. No inject. Pebbi (2026-08-17): 4.0.1 reached **Add New Baby** then Pairip on cold start. Pulled 3.2.1 (forced update), 3.4.0 and 3.5.0 (Pairip). No saved Pebbi profile. Nurture Lock: Pairip **CLOSE** only. Nubo (2026-08-18 evening): inject of finished milk/sleep/pump/bottle/pee/poop/note on the saved profile, then about 21 minutes of soak with the system HTTP proxy. New capture `results/nubo-test-20260818-soak/artifacts/captures/Nubo-finished-use-soak.mitm` (15 flows). Scan verdict `no_transmission_detected`. Marker hits were low-confidence numbers in **responses** (including a host control GET to Firebase that returned 404), not the baby name or note in a request. The 0-byte `results/nubo-test-20260817/artifacts/captures/Nubo.mitm` was kept. This still is not proof Firebase was silent.
